...
public class TicketBooth {
    ...
    // done sasaki まだわからないかもですが、インターフェースで受けましょう。ここだとMap型 by jflute (2020/04/23)
    ...
    // done sasaki 再利用するprivateメソッドは、利用する側のメソッドの下に宣言する習慣があるので移動してみてください(現場にも寄りますが) by jflute (2020/04/23)
    // done sasaki Slackのtipsスレッドで書いたように、privateの実処理メソッドは doBuy...() にした方が区別がつきやすいです by jflute (2020/04/23)
    // buyOneDayPassportやbuyTwoDayPassportを一般化
    private TicketBuyResult doBuyPassport(int handedMoney, int day) {
        // done sasaki まとまった処理ごとに概要コメントがあるのいいですね。複数の処理が一つになって一つの業務をおこなっているわけですからね by jflute (2020/04/23)
        // 売り切れチェック
        if (quantity <= 0) {
    ...
    // done sasaki getは曖昧な言葉(最終手段or本当に取るだけ)なので、できるだけ何か意味のある単語を by jflute (2020/04/23)
    ...
    private int deriveTicketPrice(int day) {
        ...
        if (price == null) {
            // done sasaki SetはGeneric型がひつようなので、Set<Integer> などで受け取ってください by jflute (2020/04/23)
            Set<Integer> daySet = PRICES.keySet();
            // done sasaki toString()しなくても、文字列と連結するだけで自然とtoString()されます by jflute (2020/04/23)
    ...
    // done sasaki [いいね] 日本語のコメントGood. ちゃんと真似て独自例外を作ってるのもGood by jflute (2020/04/23)
